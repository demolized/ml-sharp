        ...
        return this.each(function() {
            ...
            var adjustContainer = function(offset) {
                if (sliderOrientation === 'vertical') {
                    ...
                    // Use clip-path for better Safari support
                    var isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
                    if (isSafari) {
                        // Safari: use clip-path instead of clip
                        var beforeClipPath = "inset(0px " + (parseFloat(offset.w) - parseFloat(offset.cw)) + "px 0px 0px)";
                        var afterClipPath = "inset(0px 0px 0px " + offset.cw + ")";
                        beforeImg.css({
                            "clip": "auto",
                            "-webkit-clip-path": beforeClipPath,
                            "clip-path": beforeClipPath
                        });
                        afterImg.css({
                            "clip": "auto",
                            "-webkit-clip-path": afterClipPath,
                            "clip-path": afterClipPath
                        });
                    } else {
                        // Chrome/Firefox: use standard clip
